...
{:pattern #"^the crosshair is at (-?\d+) (-?\d+)$"
   ...
   {:pattern #"^the hud shows score <([A-Za-z0-9_]+)>$"
    :fn (fn [world [_ score-param] example]
          (let [expected (support/example-int example score-param "score")
                actual (:score (core/hud (:state world)))]
            (support/assert-condition (= expected actual)
                                      (str "hud score " actual " expected " expected)))
          world)}

   {:pattern #"^the hud shows multiplier <([A-Za-z0-9_]+)>$"
    :fn (fn [world [_ mult-param] example]
          (let [expected (support/example-int example mult-param "multiplier")
                actual (:multiplier (core/hud (:state world)))]
            (support/assert-condition (= expected actual)
                                      (str "hud multiplier " actual
                                           " expected " expected)))
          world)}

   {:pattern #"^the hud shows left ammo <([A-Za-z0-9_]+)>$"
    :fn (fn [world [_ ammo-param] example]
          (let [expected (support/example-int example ammo-param "left ammo")
                actual (:left-ammo (core/hud (:state world)))]
            (support/assert-condition (= expected actual)
                                      (str "hud left ammo " actual
                                           " expected " expected)))
          world)}

   {:pattern #"^the hud shows center ammo <([A-Za-z0-9_]+)>$"
    :fn (fn [world [_ ammo-param] example]
          (let [expected (support/example-int example ammo-param "center ammo")
                actual (:center-ammo (core/hud (:state world)))]
            (support/assert-condition (= expected actual)
                                      (str "hud center ammo " actual
                                           " expected " expected)))
          world)}

   {:pattern #"^the hud shows right ammo <([A-Za-z0-9_]+)>$"
    :fn (fn [world [_ ammo-param] example]
          (let [expected (support/example-int example ammo-param "right ammo")
                actual (:right-ammo (core/hud (:state world)))]
            (support/assert-condition (= expected actual)
                                      (str "hud right ammo " actual
                                           " expected " expected)))
          world)}
   ...
   {:pattern #"^the hud shows living cities <([A-Za-z0-9_]+)>$"
    :fn (fn [world [_ count-param] example]
          (let [expected (support/example-int example count-param "living cities")
                actual (:living-cities (core/hud (:state world)))]
            (support/assert-condition (= expected actual)
                                      (str "hud living cities " actual
                                           " expected " expected)))
          world)}

   {:pattern #"^the hud shows bonus cities <([A-Za-z0-9_]+)>$"
    :fn (fn [world [_ count-param] example]
          (let [expected (support/example-int example count-param "bonus cities")
                actual (:bonus-cities (core/hud (:state world)))]
            (support/assert-condition (= expected actual)
                                      (str "hud bonus cities " actual
                                           " expected " expected)))
          world)}
