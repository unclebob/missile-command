...
(defn- wave-schedule-hooks
  "Spawn hooks for wave-schedule activation (ballistics + specials)."
  []
  {:wave wave
...
(defn start-wave-attack
  ...
  Public for tests and hosts that need to stage a specific salvo."
  [state k]
  (let [n (attacks-per-wave)
        k (long k)
        m (waves/schedule-metrics-for-state state (wave state))
        ballistic (long (:enemy-count m 0))
        ;; Final attack uses full wave-schedule (ballistics + specials).
        state (if (= k n)
                (wave-schedule/activate state (wave-schedule-hooks))
                (-> state
                    (assoc :flyers [])
